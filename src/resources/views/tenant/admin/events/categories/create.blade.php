...
<div class="space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Add a new event category</p>
        </div>
    </div>

    <form action="{{ url('/admin/events/categories') }}" method="POST" class="max-w-2xl">
        @csrf
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-6">
            ...
            <div>
                ...
                <div class="mt-1 flex items-center space-x-3">
                    <input type="color" name="color" id="color" value="{{ old('color', '#3b82f6') }}" required class="h-10 w-20 rounded border-gray-300">
                    <input type="text" name="color_hex" id="color_hex" value="{{ old('color', '#3b82f6') }}" pattern="^#[0-9A-Fa-f]{6}$" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>
        ...
        <div class="mt-6 flex justify-end space-x-3">
            ...
            <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">Create Category</button>
        </div>
    </form>
</div>
