...
<div class="space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Add Room</h2>
        </div>
    </div>

    <form action="{{ url('/admin/hostel/rooms') }}" method="POST" class="max-w-2xl">
        @csrf
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="hostel_id" id="hostel_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                        ...
                        @foreach($hostels as $hostel)
                            <option value="{{ $hostel->id }}" {{ old('hostel_id', request('hostel_id')) == $hostel->id ? 'selected' : '' }}>{{ $hostel->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="room_number" id="room_number" value="{{ old('room_number') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="capacity" id="capacity" value="{{ old('capacity') }}" min="1" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>

                <div>
                    <label for="floor" class="block text-sm font-medium text-gray-700">Floor</label>
                    <input type="text" name="floor" id="floor" value="{{ old('floor') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="e.g., Ground, 1st, 2nd">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="facilities" id="facilities" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="e.g., AC, WiFi, Attached Bathroom">{{ old('facilities') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 flex justify-end space-x-3">
            ...
            <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">Save Room</button>
        </div>
    </form>
</div>
@endsection
