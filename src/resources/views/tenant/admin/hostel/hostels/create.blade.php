...
<div class="space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Add Hostel</h2>
        </div>
    </div>

    <form action="{{ url('/admin/hostel/hostels') }}" method="POST" class="max-w-3xl">
        @csrf
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Total number of beds</p>
                </div>
                ...
                <div>
                    ...
                    <select name="warden_id" id="warden_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                        ...
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('warden_id') == $teacher->id ? 'selected' : '' }}>{{ $teacher->full_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="contact_number" id="contact_number" value="{{ old('contact_number') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>
        ...
        <div class="mt-6 flex justify-end space-x-3">
            ...
            <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">Save Hostel</button>
        </div>
    </form>
</div>
@endsection
