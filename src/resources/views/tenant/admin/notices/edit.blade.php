...
<div class="space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Edit Notice</h2>
            <p class="mt-1 text-sm text-gray-500">{{ $notice->title }}</p>
        </div>
    </div>

    <form action="{{ url('/admin/notices/' . $notice->id) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $notice->title) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>
                ...
                <div>
                    ...
                    <select name="notice_type" id="notice_type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                        ...
                        <option value="announcement" {{ old('notice_type', $notice->notice_type) == 'announcement' ? 'selected' : '' }}>Announcement</option>
                        <option value="circular" {{ old('notice_type', $notice->notice_type) == 'circular' ? 'selected' : '' }}>Circular</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $notice->start_date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>

                <div>
                    <label for="end_date" class="block text-sm font-medium text-gray-700">End Date</label>
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $notice->end_date ? $notice->end_date->format('Y-m-d') : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">{{ old('content', $notice->content) }}</textarea>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-md">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="delete_attachments[]" value="{{ $attachment->id }}" class="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded">
        ...
        <div class="mt-6 flex justify-end space-x-3">
            ...
            <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">Update Notice</button>
        </div>
    </form>
</div>
@endsection
