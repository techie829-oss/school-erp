...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        ...
        <div class="mt-4 flex md:mt-0 md:ml-4">
            <a href="{{ url('/admin/subjects') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                Cancel
            </a>
        </div>
    </div>
    ...
    <form action="{{ url('/admin/subjects/' . $subject->id) }}" method="POST">
        ...
        <div class="bg-white shadow rounded-lg p-4 sm:p-6 space-y-4 sm:space-y-6">
            <div>
                ...
                <input type="text" name="subject_name" id="subject_name" value="{{ old('subject_name', $subject->subject_name) }}" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('subject_name') border-red-300 @enderror">
            </div>
            ...
            <div>
                ...
                <select name="subject_type" id="subject_type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('subject_type') border-red-300 @enderror">
                    ...
                    <option value="extra_curricular" {{ old('subject_type', $subject->subject_type) == 'extra_curricular' ? 'selected' : '' }}>Extra Curricular</option>
                </select>
            </div>

            <div>
                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                <textarea name="description" id="description" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">{{ old('description', $subject->description) }}</textarea>
            </div>
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-3 mt-6">
            <a href="{{ url('/admin/subjects') }}" class="w-full sm:w-auto inline-flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                Cancel
            </a>
