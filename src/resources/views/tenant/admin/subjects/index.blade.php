...
<div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg p-4">
        <form method="GET" action="{{ url('/admin/subjects') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span class="px-2 py-1 text-xs font-medium rounded-full {{ $subject->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                    ...
                    @if($subject->description)
                        <p class="text-xs text-gray-600 mt-2">{{ Str::limit($subject->description, 60) }}</p>
                    @endif

                    <div class="mt-3 flex justify-end space-x-2">
                        <a href="{{ url('/admin/subjects/' . $subject->id . '/edit') }}" class="text-primary-600 hover:text-primary-900 text-xs font-medium">Edit</a>
                        <form action="{{ url('/admin/subjects/' . $subject->id) }}" method="POST" onsubmit="return confirm('Delete this subject?')" class="inline">
                            @csrf
                            @method('DELETE')
