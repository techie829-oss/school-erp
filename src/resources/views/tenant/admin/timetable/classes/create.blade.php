...
<div class="space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Create Timetable</h2>
            <p class="mt-1 text-sm text-gray-500">Create a new class timetable</p>
        </div>
    </div>

    <form action="{{ url('/admin/timetable/classes') }}" method="POST" id="timetableForm">
        @csrf
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="academic_year" id="academic_year" value="{{ old('academic_year', date('Y') . '-' . (date('Y') + 1)) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="e.g., 2024-25">
                </div>

                <div>
                    <label for="term" class="block text-sm font-medium text-gray-700">Term</label>
                    <input type="text" name="term" id="term" value="{{ old('term') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="e.g., First Term">
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody id="periodsTableBody" class="bg-white divide-y divide-gray-200">
                        <!-- Period rows will be added here -->
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="mt-6 flex justify-end space-x-3">
            ...
            <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">Create Timetable</button>
        </div>
    </form>
</div>
