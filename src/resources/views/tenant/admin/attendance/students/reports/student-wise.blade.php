...
<div class="space-y-6">
    ...
    <!-- Student Info Card -->
    <div class="bg-gradient-to-r from-primary-500 to-primary-600 shadow-lg rounded-lg overflow-hidden">
        <div class="px-6 py-8 sm:flex sm:items-center sm:justify-between">
            <div class="sm:flex sm:items-center">
                @if($data['student']->photo)
                    <img class="h-20 w-20 rounded-full ring-4 ring-white" src="{{ $data['student']->photo_url }}" alt="">
                @else
                ...
                <div class="mt-4 sm:mt-0 sm:ml-6">
                    ...
                    <p class="text-sm text-primary-100">
                        ...
                        Class: {{ $data['student']->currentEnrollment->schoolClass->class_name ?? 'N/A' }} - {{ $data['student']->currentEnrollment->section->section_name ?? 'N/A' }}
                    </p>
                </div>
            </div>
            <div class="mt-5 sm:mt-0 text-center sm:text-right">
                ...
                <div class="text-sm text-primary-100">Overall Attendance</div>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-6">
        <div class="bg-white overflow-hidden shadow rounded-lg">
            <div class="p-4">
                ...
                <dd class="mt-1 text-2xl font-semibold text-gray-900">{{ $data['summary']['total_days'] ?? 0 }}</dd>
            </div>
        </div>
        <div class="bg-white overflow-hidden shadow rounded-lg">
            <div class="p-4">
