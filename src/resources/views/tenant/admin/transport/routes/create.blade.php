...
<div class="space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Add Route</h2>
        </div>
    </div>

    <form action="{{ url('/admin/transport/routes') }}" method="POST" class="max-w-3xl">
        @csrf
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="route_number" id="route_number" value="{{ old('route_number') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="start_location" id="start_location" rows="2" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">{{ old('start_location') }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label for="end_location" class="block text-sm font-medium text-gray-700">End Location <span class="text-red-500">*</span></label>
                    <textarea name="end_location" id="end_location" rows="2" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">{{ old('end_location') }}</textarea>
                </div>

                <div>
                    <label for="distance" class="block text-sm font-medium text-gray-700">Distance (km)</label>
                    <input type="number" name="distance" id="distance" value="{{ old('distance') }}" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>

                <div>
                    <label for="base_fare" class="block text-sm font-medium text-gray-700">Base Fare (₹)</label>
                    <input type="number" name="base_fare" id="base_fare" value="{{ old('base_fare', 0) }}" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>
        ...
        <div class="mt-6 flex justify-end space-x-3">
            ...
            <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">Save Route</button>
        </div>
    </form>
</div>
