    ...
    <form action="{{ url('/admin/departments/' . $department->id) }}" method="POST">
        ...
        <div class="bg-white shadow rounded-lg p-4 sm:p-6 space-y-4 sm:space-y-6">
            ...
            <div>
                ...
                <select name="head_teacher_id" id="head_teacher_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ old('head_teacher_id', $department->head_teacher_id) == $teacher->id ? 'selected' : '' }}>
                            {{ $teacher->full_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">{{ old('description', $department->description) }}</textarea>
            </div>
