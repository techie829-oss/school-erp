...
@section('title', 'No Dues Certificates')
...
<div class="space-y-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">No Dues Certificates</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div class="flex-1 min-w-0">
            ...
            <p class="mt-1 text-sm text-gray-500">
                Generate and manage no-dues certificates for students
            </p>
        </div>
        <div class="mt-4 flex md:mt-0 md:ml-4 space-x-3">
            @if(request('class_id'))
            <a href="{{ url('/admin/fees/no-dues/bulk-actions?' . http_build_query(request()->all())) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
            ...
            @endif
            <a href="{{ url('/admin/fees/no-dues/generate') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">
                <svg class="-ml-1 mr-2 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white shadow rounded-lg p-4">
        <form method="GET" action="{{ url('/admin/fees/no-dues') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Certificate number, student name, admission number..."
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                </div>
            </div>

            <div class="flex justify-end space-x-3">
                <a href="{{ url('/admin/fees/no-dues') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                    Clear
                </a>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            @php
                                $clearances = [];
                                if($certificate->fee_clearance) $clearances[] = 'Fee';
                                if($certificate->library_clearance) $clearances[] = 'Library';
                                if($certificate->lab_clearance) $clearances[] = 'Lab';
                                if($certificate->sports_clearance) $clearances[] = 'Sports';
                                if($certificate->hostel_clearance) $clearances[] = 'Hostel';
                            @endphp
                            {{ implode(', ', $clearances) ?: 'None' }}
                        </td>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Generate no-dues certificates for students</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
