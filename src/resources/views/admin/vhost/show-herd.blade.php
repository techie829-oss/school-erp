...
@section('title', 'View Herd Configuration')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-secondary-900">Herd Configuration</h1>
            <p class="text-secondary-600 mt-1">Current Herd main configuration file</p>
        </div>
        <div class="flex space-x-3">
            ...
            <a href="{{ route('admin.vhost.herd.edit') }}" class="btn-primary">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <div class="flex items-center space-x-4">
                <h2 class="text-lg font-semibold text-secondary-900">Herd Configuration File</h2>
                <div class="flex items-center space-x-2">
        ...
        <!-- Code Display -->
        <div class="relative">
            <pre id="herd-config-content" class="p-4 font-mono text-sm bg-secondary-50 overflow-x-auto" style="min-height: 600px; white-space: pre-wrap;">{{ $content }}</pre>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div>
                <h3 class="text-sm font-medium text-secondary-700 mb-2">TLD</h3>
                <p class="text-2xl font-bold text-primary-600">{{ json_decode($content, true)['tld'] ?? 'N/A' }}</p>
                <p class="text-xs text-secondary-600">Top level domain</p>
            </div>
            <div>
                <h3 class="text-sm font-medium text-secondary-700 mb-2">Project Paths</h3>
                <p class="text-2xl font-bold text-accent-600">{{ count(json_decode($content, true)['paths'] ?? []) }}</p>
                <p class="text-xs text-secondary-600">Configured paths</p>
            </div>
            <div>
                <h3 class="text-sm font-medium text-secondary-700 mb-2">PHP Version</h3>
                <p class="text-2xl font-bold text-success-600">{{ json_decode($content, true)['php']['version'] ?? 'N/A' }}</p>
                <p class="text-xs text-secondary-600">PHP version</p>
            </div>
            <div>
                <h3 class="text-sm font-medium text-secondary-700 mb-2">Loopback</h3>
                <p class="text-2xl font-bold text-info-600">{{ json_decode($content, true)['loopback'] ?? 'N/A' }}</p>
                <p class="text-xs text-secondary-600">Loopback IP</p>
            </div>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="{{ route('admin.vhost.herd.edit') }}" class="flex items-center p-4 border border-gray-200 rounded-lg hover:bg-secondary-50 transition-colors">
                ...
                <div>
                    ...
                    <p class="text-xs text-secondary-600">Modify Herd settings</p>
                </div>
            </a>
            ...
            <button onclick="downloadConfig()" class="flex items-center p-4 border border-gray-200 rounded-lg hover:bg-secondary-50 transition-colors">
                ...
                <div>
                    ...
                    <p class="text-xs text-secondary-600">Save as .json file</p>
                </div>
            </button>
        </div>
    </div>
...
function copyToClipboard() {
    const content = document.getElementById('herd-config-content').textContent;
