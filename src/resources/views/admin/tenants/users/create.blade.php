        ...
        <form method="POST" action="{{ route('admin.tenants.users.store', $tenant) }}">
            ...
            <div class="mb-6">
                ...
                <select id="admin_type"
                        name="admin_type"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('admin_type') border-red-500 @enderror"
                        required>
                    <option value="">Select admin type</option>
                    <option value="super_admin" {{ old('admin_type') === 'super_admin' ? 'selected' : '' }}>Super Admin</option>
                    <option value="super_manager" {{ old('admin_type') === 'super_manager' ? 'selected' : '' }}>Super Manager</option>
                    <option value="school_admin" {{ old('admin_type') === 'school_admin' ? 'selected' : '' }}>School Admin</option>
                </select>
                @error('admin_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
