...
@section('title', 'SMS & Email Configuration')
@section('page-title', 'SMS & Email Configuration')
@section('page-description', 'Configure SMS and Email settings for ' . ($tenant->data['name'] ?? 'Tenant'))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="card p-6">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ $tenant->data['name'] ?? 'Tenant' }} - SMS & Email Configuration</h2>
            <p class="text-sm text-gray-600 mt-2">Configure SMS and email notification settings for this tenant. Only superadmin can configure these settings.</p>
        </div>

        <form action="{{ route('admin.tenants.settings.notifications.update', $tenant) }}" method="POST" class="space-y-6">
            @csrf
