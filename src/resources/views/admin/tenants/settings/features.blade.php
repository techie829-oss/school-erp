...
@section('page-description', 'Enable or disable modules for ' . ($tenant->data['name'] ?? 'Tenant'))

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('admin.tenants.show', $tenant) }}" class="text-blue-600 hover:text-blue-800">
            ← Back to Tenant
        </a>
    </div>
    ...
    <div class="card p-6">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ $tenant->data['name'] ?? 'Tenant' }} - Module Settings</h2>
            <p class="text-sm text-gray-600 mt-2">Enable or disable specific modules for this tenant. Only superadmin can configure these settings.</p>
        </div>

        <form action="{{ route('admin.tenants.settings.features.update', $tenant) }}" method="POST" class="space-y-6">
            @csrf

            <div class="space-y-4">
                <p class="text-sm text-gray-600">Enable or disable specific modules for this institution. Disabled modules will not be accessible to users.</p>
            ...
            <div class="flex justify-end pt-4 border-t border-gray-200">
                <a href="{{ route('admin.tenants.show', $tenant) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 mr-3">
                    Cancel
                </a>
